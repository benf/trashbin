...
int main(int argc, char **argv) {
	...
	const char base[] = "dc=lab,dc=nsmlab,dc=et,dc=hs-wismar,dc=de";
...
//	char *passwd_attrs[] = {"name", "uidNumber", "gidNumber", "displayName", "unixHomeDirectory", "loginShell", NULL};
	//char *passwd_attrs[] = {"uid",  "uidNumber", "gidNumber", "cn", "homeDirectory", "loginShell", NULL};
...
void print_entries(LDAP *ld, LDAPMessage *res) {
	...
	/*
	   BerElement *ber;
	   char *attribute = ldap_first_attribute(ld, entry, &ber);
	   print_values(ld, entry, attribute);
	   struct berval **values = ldap_get_values_len(ld, entry, attribute);
	   int num = ldap_count_values_len(values);
	...
	/*
	   while ((entry = ldap_next_entry(ld, entry)) != NULL) {
	   values = ldap_get_values_len(ld, entry, "objectClass");
	   num = ldap_count_values_len(values);
	   for (i = 0; i<num; ++i)
	   printf("%s\n", values[i]->bv_val);
	   ldap_value_free_len(values);
